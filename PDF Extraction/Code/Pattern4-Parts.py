...
example_file = "C:/Users/ASUS/Desktop/3. Parts Pattern/1938__04.pdf"
folder = ""
...
def parts_sections_list(Lines, actDate):
    list_of_parts = []
    list_of_subheadings = []
    list_of_sections = []
    omittedSectionList = []
    ...
    for i, line in enumerate(Lines):
        ...
        if regex.search(r"\bP\s*A\s*R\s*T\b", line[3]) is not None or re.search("^\[\s*(Omitted|Repealed)\s*\.?\s*\]\s*\.?$", line[3], flags=re.I) is not None:
            ...
            if re.search("(Omitted|Repealed)", line[3], flags=re.I) is not None or re.search("(Rep.)", line[3]) is not None:
                ...
                part_number = re.sub(r'[^\w\s]', '', splitted[0]).strip()
                if len(splitted) > 1:
                    part_name = splitted[1].strip()
                else:
                    part_name = ''
            ...
            while True:
                ...
                if re.search("(Omitted|Repealed)", temp_line, flags=re.I) is not None:
                    ...
                    isOmitted = True                    
                if re.search("^[0-9]+\-?[A-Z]*\s*\.\s*.*", Lines[i+inc][3]) is not None:
                    break                    
                    ...
                    inc += 1
                    continue                    
                elif not temp_line.isupper() or regex.search(r"^([A-Z]\s?\.?\s?\p{Pd}|(C\s*H\s*A\s*P\s*T\s*E\s*R){e<3}.*)", temp_line) is not None or regex.search(roman_pattern, temp_line) is not None:
                    #isSubHeading = True
                    break
                else:
                    if isOmitted:
                        ...
                        part_number = line[3]
                        for j in range(inc):
                            part_number += ' ' + Lines[i+j+1][3]
                        break
                    else:
                        part_name = part_name + ' ' + Lines[i+inc][3]
                    indices.append(i+inc)
                    inc += 1

            d[key1] = {'ID': part_number.strip(), 'Name': part_name.strip()}
            list_of_parts.append([part_number.strip(), part_name.strip()])
            continue                
        ...
        if flag:
            if re.search("^[0-9]+\-?[A-Z]*\s?\.\s*.*", line[3]) is not None:
                ...
                if re.search("^\[?\s*(Omitted|Repealed)\s*\.?\s*\]\s*\.?$", sec_name, flags=re.I) is not None:      # for omitted and repealed
                    ...
                    while True:
                        if sec_name[-1]!='.':
                            if re.search("^[0-9]+\-?[A-Z]*\s*\.\s*.*", Lines[i+inc][3]) is None and regex.search(r"\b(SCHEDULE|APPENDIX|ANNEXURE){e<3}\b", Lines[i+inc][3], flags=regex.I) is None and regex.search(r"((C\s*H\s*A\s*P\s*T\s*E\s*R){e<3}|\bP\s*A\s*R\s*T\b)", Lines[i+inc][3]) is None and not Lines[i+inc][3].isupper() and regex.search(r"^\p{P}*Subject to Verification", Lines[i+inc][3], flags=regex.I) is None:
                ...
                if i not in indices and (ratio>.95 or temp_line.isupper() or regex.search(r"^([A-Z]\s*\.?\s?\p{Pd}|(C\s*H\s*A\s*P\s*T\s*E\s*R){e<3}.*)", temp_line) is not None or regex.search(roman_pattern, temp_line) is not None):
    ...
    return list_of_sections, omittedSectionList, schedule_index+1, list_of_parts, d, list_of_subheadings
...
list_of_sections, omittedSectionList, start_schedule_index, list_of_parts, pdf_dict['Parts'], list_of_subheadings = parts_sections_list(Lines, actDate)
...
for i, schedule in enumerate(list_of_schedules):
    ...
    list_of_schedules[i] = schedule.strip(punctuation)
...
def has_section(index, Lines, section, omittedSectionList):
    seenPunct = False
    clean_line = remove_superscript(Lines[index]) #CHANGED
    ...
    for num, section in enumerate(list_of_sections):
        ...
        if last_section:
            ...
            while i < len(Lines):
                ...
                if flag and (has_section(i, Lines, list_of_sections[num+1], omittedSectionList) or regex.search(r"^\[?\s*(\bP\s*A\s*R\s*T\b).*", clear_references(line[3])) is not None or in_subheadings(clear_references(line[3]), list_of_subheadings)):    # for the line where next section starts
                    if len(string_indent[1].strip()) != 0:
                        level = indent.index(string_indent[0])
                        arg_list.append(Node(string_indent[1].strip(), level))
                    string_indent[0] = -1
...
for i in pdf_dict['Parts'].keys():
    if 'Sections' in pdf_dict['Parts']:
        for j in pdf_dict['Parts']['Sections'].keys():
            pdf_dict['Parts']['Sections'][j] = sectionContent[j]
    
    if 'Sections' in pdf_dict['Parts'][i]:
        for j in pdf_dict['Parts'][i]['Sections'].keys():
            pdf_dict['Parts'][i]['Sections'][j] = sectionContent[j]
    
    if 'Subheadings' in pdf_dict['Parts'][i]:
        for j, item in enumerate(pdf_dict['Parts'][i]['Subheadings']):
            if 'Sections' in pdf_dict['Parts'][i]['Subheadings'][j].keys():
                for k in pdf_dict['Parts'][i]['Subheadings'][j]['Sections'].keys():
                    pdf_dict['Parts'][i]['Subheadings'][j]['Sections'][k] = sectionContent[k]
