...
example_file = "C:/Users/ASUS/Desktop/Thesis/2. Unnamed Chapter Pattern/A1869-4 (schedule of forms).pdf"
folder = ""
...
def chapters_sections_list(Lines, actDate):
    ...
    for i, line in enumerate(Lines):
        ...
        if regex.search(r"^(ARRANGEMENT\s?OF\s?SECTION[S]?|C\s*O\s*N\s*T\s*E\s*N\s*T\s*S){e<4}$", line[3], flags=regex.I) is not None:
            flag = True
            continue
            
        if (re.search("^(SECTION[S]?\.?)$", line[3]) is not None) or (visitPreamble and re.search("^(PREAMBLE\.?)$", line[3], flags=re.I) is not None) or (regex.search(r"^\p{P}*Subject to Verification", line[3], flags=regex.I) is not None):
            continue
        ...
        if flag:
            if re.search("^[0-9]+\-?[A-Z]*\s*\.\s*.*", line[3]) is not None:
                ...
                if re.search("^\[?\s*(Omitted|Repealed)\s*\.?\s*\]\s*\.?$", sec_name, flags=re.I) is not None:      # for omitted and repealed
                    ...
                    while True:
                        if sec_name[-1]!='.':
                            if re.search("^[0-9]+\-?[A-Z]*\s*\.\s*.*", Lines[i+inc][3]) is None and regex.search(r"\b(SCHEDULE|APPENDIX|ANNEXURE){e<3}\b", Lines[i+inc][3], flags=regex.I) is None and not Lines[i+inc][3].isupper() and regex.search(r"^\p{P}*Subject to Verification", Lines[i+inc][3], flags=regex.I) is None:
                ...
                if i not in indices and (ratio>.95 or temp_line.isupper() or regex.search(r"^([A-Z]|\bPART\b.*)\s*\.?\s?\p{Pd}", temp_line) is not None or regex.search(roman_pattern, temp_line) is not None):
                    ...
                    key_count += 1
                    key1 = str(key_count)
                    d[key1] = {'Name': subHeading_NAME.strip(), 'Sections': {}}
                    list_of_subheadings.append(subHeading_NAME.strip())
                    isSubHeading = True
    
                
    return list_of_sections, omittedSectionList, schedule_index+1, d, list_of_subheadings
...
list_of_sections, omittedSectionList, start_schedule_index, pdf_dict['Chapters'], list_of_subheadings = chapters_sections_list(Lines, actDate)
    ...
    for num, section in enumerate(list_of_sections):
        ...
        if last_section:
            ...
            while i < len(Lines):
                ...
                if flag and (has_section(i, Lines, list_of_sections[num+1], omittedSectionList) or in_subheadings(clear_references(line[3]), list_of_subheadings)):    # for the line where next section starts
                    if len(string_indent[1].strip()) != 0:
                        level = indent.index(string_indent[0])
                        arg_list.append(Node(string_indent[1].strip(), level))
                    string_indent[0] = -1
